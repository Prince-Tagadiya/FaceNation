        ...
        <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-blue-500/30">
            ...
            {viewUser && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
                    <div className="bg-slate-900 border border-slate-700 rounded-lg w-full max-w-lg shadow-2xl relative">
                        <button
                        ...
                        <div className="p-6 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="text-[10px] uppercase text-slate-500 font-mono">Official Role</label>
                                    <div className="mt-1">
                                        <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${viewUser.role === 'Investigating Officer' ? 'bg-amber-900/20 text-amber-500 border-amber-900/30' :
                                            viewUser.role === 'Control Room Operator' ? 'bg-cyan-900/20 text-cyan-500 border-cyan-900/30' :
                                                viewUser.role === 'System Admin' ? 'bg-purple-900/20 text-purple-500 border-purple-900/30' :
                                                    'bg-slate-800 text-slate-400'
                                            }`}>{viewUser.role}</span>
                                    </div>
                                </div>
                                <div className="col-span-2">
                                    <label className="text-[10px] uppercase text-slate-500 font-mono">Contact Email</label>
                                    <div className="text-slate-300 font-mono flex items-center gap-2">
                                        {viewUser.email}
                                    </div>
                                </div>
                                <div>
                                    <label className="text-[10px] uppercase text-slate-500 font-mono">Account Status</label>
                                    <div className="mt-1">
                                        {viewUser.active ?
                                            <span className="text-green-500 text-sm flex items-center gap-2 font-bold"><CheckCircle size={14} /> Active</span> :
                                            <span className="text-red-500 text-sm flex items-center gap-2 font-bold"><Ban size={14} /> Disabled</span>
                                        }
                                    </div>
                                </div>
                                <div>
                                    <label className="text-[10px] uppercase text-slate-500 font-mono">Registration Date</label>
                                    <div className="text-slate-400 text-sm font-mono mt-1">{new Date(viewUser.createdAt).toLocaleDateString()}</div>
                                </div>
                                {viewUser.createdBy && (
                                    <div className="col-span-2 bg-slate-950 p-3 rounded border border-slate-800">
                                        <label className="text-[10px] uppercase text-slate-500 font-mono block mb-1">Registered By (Admin UID)</label>
                                        <div className="text-slate-400 text-[10px] font-mono break-all">{viewUser.createdBy}</div>
                                    </div>
                                )}
                            </div>
                        </div>

                        <div className="p-4 bg-slate-950/50 rounded-b-lg border-t border-slate-800 flex justify-end">
                            <button
                                onClick={() => setViewUser(null)}
                                className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold rounded uppercase tracking-wider transition-colors"
                            >
                                Close Record
                            </button>
                        </div>
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div className="bg-slate-900 border border-slate-800 p-6 rounded-lg relative overflow-hidden group">
                                    ...
                                    <div className="text-slate-400 text-xs font-mono uppercase tracking-widest mb-2">Total Cases Processed <span className="text-[9px] text-slate-600 bg-slate-800 px-1 py-0.5 rounded ml-1">(MOCK)</span></div>
                                ...
                                <div className="bg-slate-900 border border-slate-800 p-6 rounded-lg relative overflow-hidden group">
                                    ...
                                    <div className="text-slate-400 text-xs font-mono uppercase tracking-widest mb-2">Active Alerts <span className="text-[9px] text-slate-600 bg-slate-800 px-1 py-0.5 rounded ml-1">(MOCK)</span></div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                                <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-lg h-64 flex items-center justify-center text-slate-500 font-mono text-sm border-dashed">
                                    [MOCK SYSTEM LOAD ANALYTICS]
                                </div>
                                <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-lg h-64 flex items-center justify-center text-slate-500 font-mono text-sm border-dashed">
                                    [MOCK ACTIVITY MAP STREAM]
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'personnel' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                                ...
                                {/* User List and Tools */}
                                <div className="xl:col-span-2 flex flex-col gap-4 max-h-[700px]">
                                    ...
                                    <div className="flex gap-4">
                                        <div className="relative flex-1">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-slate-900 border border-slate-800 rounded pl-10 pr-4 py-3 text-sm focus:border-blue-500 outline-none text-slate-300 placeholder-slate-600 transition-colors"
                                            />
                                        </div>
                                        <div className="relative w-56">
                                            ...
                                            <select
                                                ...
                                                className="w-full bg-slate-900 border border-slate-800 rounded pl-10 pr-4 py-3 text-sm focus:border-blue-500 outline-none text-slate-300 appearance-none cursor-pointer transition-colors"
                                            >
                                    ...
                                    <div className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden flex flex-col flex-1">
                                        <div className="bg-slate-800/50 px-6 py-4 border-b border-slate-800 flex justify-between items-center">
                                            <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wider">Active Roster ({filteredUsers.length})</h3>
                                        </div>
                                                        ...
                                                        <tr key={u.uid} className="hover:bg-slate-800/30">
                                                            ...
                                                            <td className="p-4">
                                                                <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${u.role === 'Investigating Officer' ? 'bg-amber-900/20 text-amber-500 border-amber-900/30' :
                                                                    u.role === 'Control Room Operator' ? 'bg-cyan-900/20 text-cyan-500 border-cyan-900/30' :
                                                                        u.role === 'System Admin' ? 'bg-purple-900/20 text-purple-500 border-purple-900/30' :
                                                                            'bg-slate-800 text-slate-400'
                                                                    }`}>{u.role}</span>
                                                            </td>
                                                            <td className="p-4">
                                                            ...
                                                            <td className="p-4 text-right space-x-2">
                                                                <button
                                                                    ...
                                                                    className="text-blue-500 hover:bg-blue-900/20 p-1.5 rounded transition-colors"
                                                                    title="View Profile"
                                                                >
                                                                    <Info size={16} />
                                                                </button>
